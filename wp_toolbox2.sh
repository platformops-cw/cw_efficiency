...
DB_ENGINE () {
  ...
  wp db query "SELECT CONCAT('ALTER TABLE ',table_schema,'.',table_name,' engine=InnoDB;') FROM information_schema.tables WHERE engine = 'MyISAM';" --skip-column-names --skip-plugins --skip-themes --quiet > /home/master/DB_ENGINE_tables


    for A in $(cat /home/master/DB_ENGINE_tables); do
  echo $A
  #wp db query "${A}"
...
$(ColorYellow 'Choose an option:') "
        ...
        case $a in
                ...
                3) WP_DOCTOR ; ttfb_optimizations ;;
